...
class Peserta extends CI_Controller 
{
     ...
     public function index()
    {
        ...
        if(isset($_POST['search'])){
            $data['users'] = $this->users->search($_POST['search'], $role);
        }else{
            $data['users'] = $this->users->get_peserta();
        }
        
        $this->load->view('datamaster/peserta/index',$data);
    }
    ...
    public function add()
    {
       $users = $this->users;
        ...
        if(! $_POST){
        $this->load->view("datamaster/peserta/add");
        }
    ...
    public function edit($id)
    {
        ...
        if (!$peserta) show_404();
        $this->load->view("datamaster/peserta/edit", array(
                    'peserta' =>$peserta,
                    'role' => $role,
                ));
 ...
 public function delete($id=null)
	...
	{		
		...
		$data =  $this->users->get_peserta();
			
		$pdf->AddPage('L', 'A3');
		$view = $this->load->view('datamaster/peserta/pdf',[
			'peserta'=> $data,
		],true);

		$pdf->writeHTML($view);

		$pdf->Output('Daftar Peserta'.date('d-m-Y').'.pdf', 'D');
	}
	...
	{		
		...
		$filename ="Daftar Peserta".date('Y-m-d H:i:s');
