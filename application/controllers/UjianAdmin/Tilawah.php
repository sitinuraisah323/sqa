...
class Tilawah extends CI_Controller 
{
    ...
    {
        $data['tilawah'] = $this->ujian->db->where('id_program',3)->get('examquestion')->result();
        // $role = $this->session->userdata('role_id');
        // // echo "Selamat Datang User " . $data['user']['nama'];
        // if ($role == 2) {
        //     $this->load->view('program/index', $data );
        // }else{
            $this->load->view('ujian/tilawah/index', $data );
        // }
    }
    ...
    public function add()
    {
        ...
        if ($validation->run()) {
                ...
                $data = $this->ujian->db->where('id_program',3)->get('examquestion')->result();
                $this->load->view("ujian/tilawah/index", array(
                    'tilawah' =>$data,
                    'message' => $message
                ));
        ...
        if(! $_POST){
        $this->load->view("ujian/tilawah/add");
        }
  ...
  public function edit($id = null)
    {
        ...
        $this->load->view("ujian/tilawah/edit", $data);
    }
    ...
    public function delete($id)
    {
        ...
        if ($this->ujian->delete($id)) {
            ...
            redirect(site_url('ujian/tilawah', ));
        }
    }
